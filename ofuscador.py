...
class Janela(ttk.Frame):
    def __init__(self, pai):
        ...
        ttk.Label(self, text = "senha: ").grid(column = 1, row = 2, stick = E)
        self.botao = ttk.Button(self, text = "Cifrar", command = self.cifrar)
        self.botao.grid(column = 3, row = 2, stick = W)

        ttk.Entry(self, width = "30", textvariable = self.senha, show = "*").grid(column = 2, row = 2, stick = (W, E))

        self.texto = tk.Text(self, width = "80", height = "30")
        self.texto.grid(column = 1, row = 1, columnspan = 3, stick = (W, E))
